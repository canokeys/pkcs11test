...
// Set up the environment for PKCS#11 and include the official PKCS#11 header
// file.
...
inline CK_ULONG PinLength(const char *pin) {
  return pin == g_so_pin ? g_so_pin_len : static_cast<CK_ULONG>(strlen(pin));
}
...
void TestSkipped(const char *testcase, const char *test,
...
public:
  PKCS11Test() {
    ...
    const CK_RV init_rv = g_fns->C_Initialize(NULL_PTR);
    EXPECT_CKR_OK(init_rv);
    if (init_rv != CKR_OK)
      return;

    // Some PC/SC-backed modules recreate their slot table on every
    // initialize/finalize cycle.  Keep the selected token slot current while
    // preserving an explicit slot ID when the implementation keeps it valid.
    CK_SLOT_ID slots[32];
    CK_ULONG slot_count = sizeof(slots) / sizeof(slots[0]);
    if (g_fns->C_GetSlotList(CK_TRUE, slots, &slot_count) == CKR_OK &&
        slot_count > 0) {
      bool selected_slot_present = false;
      for (CK_ULONG ii = 0; ii < slot_count; ++ii) {
        if (slots[ii] == g_slot_id) {
          selected_slot_present = true;
          break;
        }
      }
  ...
  virtual ~PKCS11Test() { EXPECT_CKR_OK(g_fns->C_Finalize(NULL_PTR)); }
};
...
public:
  SessionTest() : session_(INVALID_SESSION_HANDLE) {
    ...
    if (!(slot_info.flags & CKF_TOKEN_PRESENT)) {
      std::cerr << "Need to specify a slot ID that has a token present"
                << std::endl;
    }
  }
  virtual ~SessionTest() {
  ...
  void Login(CK_USER_TYPE user_type, const char *pin) {
    CK_RV rv = g_fns->C_Login(session_, user_type, (CK_UTF8CHAR_PTR)pin,
                              PinLength(pin));
    if (rv != CKR_OK) {
      std::cerr << "Failed to login as user type " << user_type_name(user_type)
                << ", error " << rv_name(rv) << std::endl;
    }
  }
...
public:
  ReadOnlySessionTest() {
    ...
    EXPECT_CKR_OK(
        g_fns->C_OpenSession(g_slot_id, flags, NULL_PTR, NULL_PTR, &session_));
  }
};
...
// The following test fixtures perform a login, which is only appropriate if the
...
// The following test fixtures perform a login if the token flags indicate login
// is required.
...
public:
  Session() {
    EXPECT_CKR_OK(
        g_fns->C_OpenSession(g_slot_id, F, NULL_PTR, NULL_PTR, &session_));
  }
...
public:
  LoginSession(const char *pin) {
    CK_RV rv = g_fns->C_Login(Session<F>::handle(), U, (CK_UTF8CHAR_PTR)pin,
                              strlen(pin));
    if (rv != CKR_OK) {
      std::cerr << "Failed to login as user type " << user_type_name(U)
                << " with PIN '" << pin << "', error " << rv_name(rv)
                << std::endl;
    }
  }
...
typedef LoginSession<(CKF_SERIAL_SESSION | CKF_RW_SESSION), CKU_USER>
    RWUserSession;
...
public:
  ...
  ObjectAttributes(std::vector<CK_ATTRIBUTE_TYPE> &attr_types) {
    ...
    for (CK_ATTRIBUTE_TYPE attr_type : attr_types) {
      CK_ATTRIBUTE attr = {attr_type, (CK_VOID_PTR)&g_ck_true,
                           sizeof(CK_BBOOL)};
...
private:
  friend std::ostream &operator<<(std::ostream &os,
                                  const ObjectAttributes &attrobj);
...
class SecretKey {
public:
  // Create a secret key with the given list of (boolean) attributes set to
  // true.
  SecretKey(CK_SESSION_HANDLE session, const ObjectAttributes &attrs,
    ...
    CK_ATTRIBUTE public_object = {CKA_PRIVATE, (CK_VOID_PTR)&g_ck_false,
                                  sizeof(CK_BBOOL)};
    attrs_.push_back(public_object);
    CK_MECHANISM mechanism = {keygen_mechanism, NULL_PTR, 0};
    EXPECT_CKR_OK(g_fns->C_GenerateKey(session_, &mechanism, attrs_.data(),
                                       attrs_.size(), &key_));
  }
  ~SecretKey() {
...
public:
  ...
  KeyPair(CK_SESSION_HANDLE session, const ObjectAttributes &public_attrs,
          const ObjectAttributes &private_attrs)
      : session_(session), public_attrs_(public_attrs),
        private_attrs_(private_attrs), public_key_(INVALID_OBJECT_HANDLE),
        private_key_(INVALID_OBJECT_HANDLE) {
    CK_ULONG modulus_bits = 1024;
    CK_ATTRIBUTE modulus = {CKA_MODULUS_BITS, &modulus_bits,
                            sizeof(modulus_bits)};
    public_attrs_.push_back(modulus);
    CK_BYTE public_exponent_value[] = {0x1, 0x0, 0x1}; // 65537=0x010001
    CK_ATTRIBUTE public_exponent = {CKA_PUBLIC_EXPONENT, public_exponent_value,
                                    sizeof(public_exponent_value)};
    ...
    EXPECT_CKR_OK(g_fns->C_GenerateKeyPair(
        session_, &mechanism, public_attrs_.data(), public_attrs_.size(),
        private_attrs_.data(), private_attrs_.size(), &public_key_,
        &private_key_));
  }
  ~KeyPair() {
...
public:
  ...
  SecretKeyTest()
      : attrs_({CKA_ENCRYPT, CKA_DECRYPT}), info_(kCipherInfo[GetParam()]),
