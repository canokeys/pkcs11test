...
TEST_F(ReadOnlySessionTest, UserLoginWrongPIN) {
  EXPECT_CKR(
      CKR_PIN_INCORRECT,
      g_fns->C_Login(session_, CKU_USER, (CK_UTF8CHAR_PTR) "simply-wrong", 12));
  ...
  EXPECT_CKR_OK(g_fns->C_Login(session_, CKU_USER, (CK_UTF8CHAR_PTR)g_user_pin,
                               strlen(g_user_pin)));
...
TEST_F(ReadOnlySessionTest, UserLoginInvalid) {
  EXPECT_CKR(CKR_SESSION_HANDLE_INVALID,
             g_fns->C_Login(INVALID_SESSION_HANDLE, CKU_USER,
                            (CK_UTF8CHAR_PTR)g_user_pin, strlen(g_user_pin)));
  EXPECT_CKR(CKR_USER_TYPE_INVALID,
             g_fns->C_Login(session_, 99, (CK_UTF8CHAR_PTR)g_user_pin,
                            strlen(g_user_pin)));
}
...
TEST_F(ReadOnlySessionTest, SOLoginFail) {
  ...
  EXPECT_CKR(CKR_SESSION_READ_ONLY_EXISTS,
             g_fns->C_Login(session_, CKU_SO, (CK_UTF8CHAR_PTR)g_so_pin,
                            g_so_pin_len));
}
...
TEST_F(ReadWriteSessionTest, ReadOnlySessionSOLoginFail) {
  ...
  // Presence of the read-only session prevents SO login even though there's a
  // RW session (as login state applies across all sessions).
  EXPECT_CKR(CKR_SESSION_READ_ONLY_EXISTS,
             g_fns->C_Login(session_, CKU_SO, (CK_UTF8CHAR_PTR)g_so_pin,
                            g_so_pin_len));
...
TEST_F(ROUserSessionTest, LogoutInvalid) {
  EXPECT_CKR(CKR_SESSION_HANDLE_INVALID,
             g_fns->C_Logout(INVALID_SESSION_HANDLE));
}

TEST_F(ROUserSessionTest, UserLoginAlreadyLoggedIn) {
  EXPECT_CKR(CKR_USER_ALREADY_LOGGED_IN,
             g_fns->C_Login(session_, CKU_USER, (CK_UTF8CHAR_PTR)g_user_pin,
                            strlen(g_user_pin)));
}
...
TEST_F(RWUserSessionTest, SOLoginFail) {
  ...
  CK_RV rv =
      g_fns->C_Login(session_, CKU_SO, (CK_UTF8CHAR_PTR)g_so_pin, g_so_pin_len);
...
TEST_F(ReadWriteSessionTest, SOLogin) {
  ...
  if (!(g_token_flags & CKF_PROTECTED_AUTHENTICATION_PATH)) {
    // PKCS#11 s6.7.1: When the session is initially opened, it is in [..] the
    // "R/O Public Session" if the application has no previously open sessions
    // that are logged in
    EXPECT_EQ(CKS_RW_PUBLIC_SESSION, session_info.state);
  } else {
    // PKCS#11 s9.2: Token has a "protected authentication path", whereby a user
    // can log into the token without passing a PIN through the Cryptoki
    // library, so initial state might be logged in.
  }

  // Now login as SO.
  EXPECT_CKR_OK(g_fns->C_Login(session_, CKU_SO, (CK_UTF8CHAR_PTR)g_so_pin,
                               g_so_pin_len));
