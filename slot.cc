...
TEST_F(PKCS11Test, EnumerateSlots) {
  ...
  unique_ptr<CK_SLOT_ID, freer> slot(
      (CK_SLOT_ID *)malloc(slot_count * sizeof(CK_SLOT_ID)));
  // Retrieve slot list.
...
TEST_F(PKCS11Test, EnumerateMechanisms) {
  ...
  EXPECT_CKR_OK(
      g_fns->C_GetMechanismList(g_slot_id, NULL_PTR, &mechanism_count));
  unique_ptr<CK_MECHANISM_TYPE, freer> mechanism((CK_MECHANISM_TYPE_PTR)malloc(
      mechanism_count * sizeof(CK_MECHANISM_TYPE)));
  EXPECT_CKR_OK(
      g_fns->C_GetMechanismList(g_slot_id, mechanism.get(), &mechanism_count));
  for (int ii = 0; ii < (int)mechanism_count; ii++) {
    const CK_MECHANISM_TYPE mechanism_type = mechanism.get()[ii];
    CK_MECHANISM_INFO mechanism_info;
    EXPECT_CKR_OK(
        g_fns->C_GetMechanismInfo(g_slot_id, mechanism_type, &mechanism_info));
    if (g_verbose)
...
TEST_F(PKCS11Test, GetSlotList) {
  CK_ULONG slot_count;
  EXPECT_CKR_OK(g_fns->C_GetSlotList(CK_FALSE, NULL_PTR, &slot_count));
  unique_ptr<CK_SLOT_ID, freer> all_slots(
      (CK_SLOT_ID *)malloc(slot_count * sizeof(CK_SLOT_ID)));
  ...
  for (int ii = 0; ii < (int)slot_count; ++ii) {
    ...
    if (!(slot_info.flags & CKF_TOKEN_PRESENT)) {
      CK_TOKEN_INFO token_info;
      EXPECT_CKR(CKR_TOKEN_NOT_PRESENT,
                 g_fns->C_GetTokenInfo(slot_id, &token_info));
    }
  }
  EXPECT_CKR_OK(g_fns->C_GetSlotList(CK_TRUE, NULL_PTR, &slot_count));
  unique_ptr<CK_SLOT_ID, freer> token_slots(
      (CK_SLOT_ID *)malloc(slot_count * sizeof(CK_SLOT_ID)));
  ...
  for (int ii = 0; ii < (int)slot_count; ++ii) {
    ...
    EXPECT_CKR(CKR_ARGUMENTS_BAD,
               g_fns->C_GetTokenInfo(token_slots.get()[ii], nullptr));
  }
}
...
TEST_F(PKCS11Test, GetSlotListFailTooSmall) {
  ...
  if (slot_count > 1) {
    ...
    EXPECT_CKR(CKR_BUFFER_TOO_SMALL,
               g_fns->C_GetSlotList(CK_FALSE, all_slots.get(), &new_count));
...
TEST_F(PKCS11Test, GetSlotListTooLarge) {
  ...
  if (slot_count > 1) {
    ...
    unique_ptr<CK_SLOT_ID, freer> all_slots(
        (CK_SLOT_ID *)malloc(new_count * sizeof(CK_SLOT_ID)));
...
TEST_F(PKCS11Test, GetSlotInfoFail) {
  CK_SLOT_INFO slot_info;
  memset(&slot_info, 0, sizeof(slot_info));
  EXPECT_CKR(CKR_SLOT_ID_INVALID,
             g_fns->C_GetSlotInfo(INVALID_SLOT_ID, &slot_info));
...
TEST_F(PKCS11Test, GetTokenInfoFail) {
  ...
  memset(&token_info, 0, sizeof(token_info));
  EXPECT_CKR(CKR_SLOT_ID_INVALID,
             g_fns->C_GetTokenInfo(INVALID_SLOT_ID, &token_info));
...
TEST_F(PKCS11Test, GetMechanismListFailInvalidSlot) {
  ...
  EXPECT_CKR(
      CKR_SLOT_ID_INVALID,
      g_fns->C_GetMechanismList(INVALID_SLOT_ID, NULL_PTR, &mechanism_count));
}
...
TEST_F(PKCS11Test, GetMechanismListFailTooSmall) {
  ...
  if (mechanism_count > 1) {
    unique_ptr<CK_MECHANISM_TYPE, freer> mechanism(
        (CK_MECHANISM_TYPE_PTR)malloc(mechanism_count *
                                      sizeof(CK_MECHANISM_TYPE)));
    CK_ULONG new_count = mechanism_count - 1;
    EXPECT_CKR(
        CKR_BUFFER_TOO_SMALL,
        g_fns->C_GetMechanismList(g_slot_id, mechanism.get(), &new_count));
...
TEST_F(PKCS11Test, GetMechanismListTooLarge) {
  ...
  if (mechanism_count > 1) {
    ...
    unique_ptr<CK_MECHANISM_TYPE, freer> mechanism(
        (CK_MECHANISM_TYPE_PTR)malloc(new_count * sizeof(CK_MECHANISM_TYPE)));
    EXPECT_CKR(CKR_OK, g_fns->C_GetMechanismList(g_slot_id, mechanism.get(),
                                                 &new_count));
    EXPECT_EQ(mechanism_count, new_count);
  }
}
...
TEST_F(PKCS11Test, GetMechanismInfoInvalid) {
  ...
  EXPECT_CKR(CKR_MECHANISM_INVALID,
             g_fns->C_GetMechanismInfo(g_slot_id, CKM_VENDOR_DEFINED + 1,
                                       &mechanism_info));
}
...
TEST_F(PKCS11Test, GetMechanismInfoInvalidSlot) {
  CK_MECHANISM_INFO mechanism_info;
  EXPECT_CKR(CKR_SLOT_ID_INVALID,
             g_fns->C_GetMechanismInfo(
                 INVALID_SLOT_ID, CKM_RSA_PKCS_KEY_PAIR_GEN, &mechanism_info));
}

TEST_F(PKCS11Test, GetMechanismInfoFail) {
  CK_RV rv =
      g_fns->C_GetMechanismInfo(g_slot_id, CKM_RSA_PKCS_KEY_PAIR_GEN, NULL_PTR);
  EXPECT_TRUE(rv == CKR_ARGUMENTS_BAD || rv == CKR_FUNCTION_FAILED)
      << " rv=" << CK_RV_(rv);
}
...
TEST(Slot, NoInit) {
  ...
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_GetSlotList(CK_FALSE, NULL_PTR, &slot_count));
  CK_SLOT_INFO slot_info;
  memset(&slot_info, 0, sizeof(slot_info));
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_GetSlotInfo(g_slot_id, &slot_info));
  CK_TOKEN_INFO token_info;
  memset(&token_info, 0, sizeof(token_info));
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_GetTokenInfo(g_slot_id, &token_info));
  CK_SLOT_ID slot_id = -1;
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_WaitForSlotEvent(CKF_DONT_BLOCK, &slot_id, NULL_PTR));
  CK_ULONG mechanism_count;
  ...
  CK_MECHANISM_INFO mechanism_info;
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_GetMechanismInfo(g_slot_id, CKM_RSA_PKCS_KEY_PAIR_GEN,
                                       &mechanism_info));
  ...
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_InitToken(INVALID_SLOT_ID, (CK_UTF8CHAR_PTR)g_so_pin,
                                g_so_pin_len, label));
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_InitPIN(INVALID_SESSION_HANDLE,
                              (CK_UTF8CHAR_PTR)g_user_pin, strlen(g_user_pin)));
  EXPECT_CKR(CKR_CRYPTOKI_NOT_INITIALIZED,
             g_fns->C_SetPIN(INVALID_SESSION_HANDLE,
                             (CK_UTF8CHAR_PTR)g_user_pin, strlen(g_user_pin),
                             (CK_UTF8CHAR_PTR)g_user_pin, strlen(g_user_pin)));
}
...
TEST_F(PKCS11Test, TokenInit) {
  ...
  if (g_token_flags & CKF_PROTECTED_AUTHENTICATION_PATH) {
    if (g_verbose)
      cout << "Skipping token initialization due to protected authentication "
              "path"
           << endl;
  }
  // !!!WARNING!!! - The following line will destroy all content on the token.
  EXPECT_CKR_OK(g_fns->C_InitToken(g_slot_id, (CK_UTF8CHAR_PTR)g_so_pin,
                                   g_so_pin_len, g_token_label));

  if (!(g_token_flags & CKF_LOGIN_REQUIRED)) {
    if (g_verbose)
      cout << "Skipping restoration of PINs" << endl;
    return;
  }

  // User PIN will have been reset, so need to set it. Use a new session (which
  // also checks that the SO PIN is still OK).
  {
    RWSOSession session(g_so_pin);
    EXPECT_CKR_OK(g_fns->C_InitPIN(
        session.handle(), (CK_UTF8CHAR_PTR)g_user_pin, strlen(g_user_pin)));
  }
  // Check the user PIN is as expected.
  {
    ROSession session;
    EXPECT_CKR_OK(g_fns->C_Login(session.handle(), CKU_USER,
                                 (CK_UTF8CHAR_PTR)g_user_pin,
                                 strlen(g_user_pin)));
    g_fns->C_Logout(session.handle());
  }
  // TODO(drysdale): figure this out
  // Some tokens (OpenCryptoKi) don't do anything on InitPIN.  Instead, log in
  // with the reset user PIN and do SetPIN.
  {
    RWUserSession session(g_reset_user_pin);
    EXPECT_CKR_OK(
        g_fns->C_SetPIN(session.handle(), (CK_UTF8CHAR_PTR)g_reset_user_pin,
                        strlen(g_reset_user_pin), (CK_UTF8CHAR_PTR)g_user_pin,
                        strlen(g_user_pin)));
  }
  // Check the user PIN is as expected.
  {
...
TEST_F(PKCS11Test, TokenInitPinIncorrect) {
  ...
  EXPECT_CKR(CKR_PIN_INCORRECT,
             g_fns->C_InitToken(g_slot_id, (CK_UTF8CHAR_PTR)wrong_pin,
                                strlen(wrong_pin), g_token_label));
}
...
TEST_F(ReadOnlySessionTest, TokenInitWithSession) {
  ...
  EXPECT_CKR(CKR_SESSION_EXISTS,
             g_fns->C_InitToken(g_slot_id, (CK_UTF8CHAR_PTR)g_so_pin,
                                g_so_pin_len, g_token_label));
}
